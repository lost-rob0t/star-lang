...
unquoted-key, number, trailing-comma, or trailing-input extensions."
  (let ((index 0) (size (length text)) (value-count 0))
    (labels ((peek () (and (< index size) (char text index)))
             (skip-whitespace () (loop while (member (peek) '(#\Space #\Tab #\Return #\Newline))
                             do (incf index)))
                       ...
                       ((eql c #\\)
                        ...
                        (let ((escape (peek)))
                          ...
                          (when (eql escape #\u)
                            (dotimes (i 4)
                              (declare (ignore i))
                              (unless (and (peek) (find (peek) "0123456789abcdefABCDEF")) (fail-wire))
                              (incf index)))))
                       (t (incf index)))))
             ...
             (value (depth)
               (when (or (> depth +wire-depth-limit+)
                         (> (incf value-count) +wire-value-limit+)) (fail-wire))
               (skip-whitespace)
               (case (peek)
                 (#\" (string-token))
                 (#\{
                  (incf index) (skip-whitespace)
                  (let ((keys (make-hash-table :test #'equal)))
                    (unless (eql (peek) #\})
                      (loop
                        ...
                        (skip-whitespace) (take #\:) (value (1+ depth)) (skip-whitespace)
