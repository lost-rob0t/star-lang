...
(test json-file-writer-normalizes-relative-roots
  "Relative writer roots resolve against the process working directory."
  (let* ((relative-root
           (format nil "star-artifact-relative-~A/"
                   (symbol-name (gensym "RUN-"))))
         (absolute-root
           (merge-pathnames
            (uiop:ensure-directory-pathname (pathname relative-root))
            (uiop:getcwd))))
    (unwind-protect
         (let* ((runtime (make-runtime))
                (actor
                ...
                (expected
                  (merge-pathnames
                   "github-members/ada.json"
                   absolute-root)))
           (is (eq :written (json-file-result-status result)))
           (is (equal (truename expected) (truename path))))
      (delete-test-root absolute-root))))
